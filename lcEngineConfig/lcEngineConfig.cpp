// lcEngineConfig.cpp : 定义应用程序的入口点。
//

#include "stdafx.h"
#include "lcEngineConfig.h"
#include <shlwapi.h>
        ...
        if (hMap)
        {
            ...
            if (pBegin)
            {
                ...
                if (send_process)
                {
                    ...
                    while (*(pSea2++) != 0xC3) // look for ret
                    {
                    }
                    while(*(--pSea2) != 0xE8)  // look for call xxxx
                    {
                    }
